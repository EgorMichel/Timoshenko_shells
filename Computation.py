...
def Compute_Lax_Frid(data, Ax, Ay, dt, dx, dy, steps=100):
    DATA = np.zeros((steps, data.shape[0], data.shape[1], data.shape[2])) # Container for data
    ...
    for _ in range(steps):
        ...
        # x_part = np.einsum('ij,klj->kli', Ax, (data[2:, 1:-1] - data[:-2, 1:-1]))
        # y_part = np.einsum('ij,klj->kli', Ay, (data[1:-1, 2:] - data[1:-1, :-2]))
...
def Compute_Lax_Vend(data, Ax, Ay, dt, dx, dy, steps=100):
    DATA = np.zeros((steps, data.shape[0], data.shape[1], data.shape[2])) # Container for data
    ...
    for _ in range(steps):
        ...
        d2u_dxdy = (data[2:, 2:] - data[2:, :-2] + data[:-2, 2:] + data[:-2, :-2])
        ...
        xy_2 = d2u_dxdy @ (Ax @ Ay + Ay @ Ax) * dt**2 / (4 * dx * dy)
